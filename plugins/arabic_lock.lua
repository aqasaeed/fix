...
local function run(msg, matches)
  ...
  if data[tostring(msg.to.id)]['settings']['lock_arabic'] then
    if data[tostring(msg.to.id)]['settings']['lock_arabic'] == 'yes' then
      ...
      send_large_msg("chat#id".. msg.to.id , "In questo gruppo i caratteri arabi non sono consentiti")
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] kickato (i caratteri arabi sono vietati) ")
